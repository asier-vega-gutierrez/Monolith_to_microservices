...
resource "azurerm_network_interface" "influx_ni" {
  name                = "influx_ni"
...
resource "azurerm_ssh_public_key" "influx_spk" {
  name                = "influx_spk"
  resource_group_name = var.resource_group_name
...
resource "azurerm_virtual_machine" "influx-vm" {
  name                  = "influx-vm"
  location              = var.resource_group_location
  resource_group_name   = var.resource_group_name
  network_interface_ids = [azurerm_network_interface.influx_ni.id]
  ...
  os_profile_linux_config {
    ...
    ssh_keys {
      ...
      key_data = azurerm_ssh_public_key.influx_spk.public_key
    }
  }
}
