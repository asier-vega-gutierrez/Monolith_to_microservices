...
class MouldDataExtractor():
    ...
    def _iterate_and_inject(self):
        ...
        while self._run:
            ...
            '''self._communication_manager.data_distribution(f"{self._config.mould_data_topic}_l{self._line}", json_data)'''
            data_distribution(f"{self._config.mould_data_topic}_l{self._line}", json_data)
