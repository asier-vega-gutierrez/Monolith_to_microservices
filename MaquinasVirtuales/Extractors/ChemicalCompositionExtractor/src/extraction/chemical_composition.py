...
class ChemicalCompositionExtractor():
    ...
    def log_info(self, message):
        ...
        logging.basicConfig(filename='logs/ChemicalCompositionExtractor.txt', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    def _iterate_and_inject(self):
        ...
        while self._run:
            ...
            self.log_info(f"Notify chemical composition - ID = {chemical_composition_to_send.id}")
            ...
            '''self._communication_manager.data_distribution(self._config.chemical_composition_topic, json_data)'''
            data_distribution(self._config.chemical_composition_topic, json_data)

            time_for_next = random.randint(60, 120)
            print(f"Time waiting to next chemical composition message: {time_for_next} s.")
            self.log_info(f"Time waiting to next chemical composition message: {time_for_next} s.")
            sleep(time_for_next) 
