...
echo Finding the Process ID (PID) of Grpc:
for /f "tokens=2 delims=," %%a in ('wmic process where "CommandLine like '%%Grpc%%' and Name='python.exe'" get ProcessId /format:csv') do set "PID=%%~a"
...
echo Stopping Grpc (PID: %PID%):
taskkill /F /PID %PID%
echo Grpc stopped successfully.