...
class SensorDataWriter(SubscriberBase):
    """ Class to manage the Sensor Data Load process into Relational storage """
    ...
    def notify(self, message):
        ...
        print(f"SENSORS OBSERVER: message received for line {self._line}")
        print(message)
        self.log_info(f"SENSORS OBSERVER: message received for line {self._line}")
        self.log_info(message)

        # We deserializae the JSON data
        decoded_data = SensorData(**json.loads(message))
        print(decoded_data)

        # We store the data into the relational store 
        data = self._unit_of_work.output.sensor_store.add_sensor(decoded_data)
