...
class MUCSIConsumer():
    ...
    def __init__(self):
        """ Comstructor of the class that generates the basic structures for an observer/observable pattern """

        config = PDAgentTemporlSeriesWriterConfiguration()

        my_group_id = config.group_id
        ...
        while not server_ready:
            try:
                self._consumer = KafkaConsumer(
                    client_id=f'{config.client_id}_{uuid.uuid4()}',
                ...
                server_ready = True
                print("KAFKA READY to receive temporal series data...")
            except:
                print("KAFKA NOT READY to receive temporal series data...")
