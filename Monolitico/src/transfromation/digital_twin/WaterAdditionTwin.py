from transfromation.predictions.belts_water_addition_factor import *
from transfromation.predictions.cooling_drum_water_addition import *
...
from transfromation.digital_twin.message_processors.sensors import SensorMessageProcessor
from transfromation.digital_twin.message_processors.mould import MouldMessageProcessor
...
class WaterAdditionDigitalTwin():
    ...
    def start_digital_twin_processing(self):
        ...
        if self._type.upper() == 'DRUM':
            self._data_communication_manager.add_observer(f"{self._config.mould_data_topic}_l{self._line}", self._mould_message_processor)
        else:
            self._data_communication_manager.add_observer(f"{self._config.mould_for_belts_topic}", self._mould_message_processor)
        
        # Then for sensors
        self._data_communication_manager.add_observer(f"{self._config.sensor_data_topic}_l{self._line}", self._sensor_message_processor)           
    ...
    def stop_digital_twin_processing(self): 
        ...
        # First for moulds
        if self._type.upper() == 'DRUM':
            self._data_communication_manager.remove_observer(f"{self._config.mould_data_topic}_l{self._line}", self._mould_message_processor)
        else:
            self._data_communication_manager.remove_observer(f"{self._config.mould_for_belts_topic}", self._mould_message_processor)
        
        # Then for sensors
        self._data_communication_manager.remove_observer(f"{self._config.sensor_data_topic}_l{self._line}", self._sensor_message_processor)
    ...
    def _calculate_water_addition(self):
        ...
        if self._type.upper() == 'DRUM':
            ...
            print(f"COOLING DRUM Water Addition for Line {self._line}: {response_liters}")
            ...
            addition_data = WaterAdditionData(datetime.now(timezone.utc), self._line, self._type, response_liters, data_for_prediction)
            ...
            print("COOLING DRUM Belts Addition: {}".format(self._line, response_factor))
            ...
            addition_data = WaterAdditionData(datetime.now(timezone.utc), self._line, self._type, response_factor, data_for_prediction)
