...
class WaterDrumPredictionTemporalSeriesWriter(ObserverBase):
    """ Class to manage the Water Prediction Load process into Tempral Series storage """
    ...
    def start_observe_and_load_data(self):
        ...
        self._communication_manager.add_observer(f"{self._config.water_prediction_drum_topic}_l{self._line}", self)

    def stop_observe_and_load_data(self):
        """ Method to stop listen new messages  """
        
        self._communication_manager.remove_observer(f"{self._config.water_prediction_drum_topic}_l{self._line}", self)
    ...
    def notify(self, message):
        ...
        print(f"PREDICTION OBSERVER (Temporal Series): message received for line {self._line}")
        print(message)
